...
var tic = function(){
	...
	return Promise.resolve(q)
		...
		.then(function(record){
			return translate(translateOptions(record, true))
				.catch(function(error){
					...
					let update = [{_id: record._id}, {'$set': {error: {out: error}, modified: Date.now()}}, {multi: false}];
					return Promise.reject(update);
				})
				.then(function(result){
					//console.log('[3] transleted-ok:', result.$(':root').html());
					let out = record.out = out_divs(result.$);
					let update = [{'_id': uf('%s', record._id)}, {'$set': {out: out, modified: Date.now()}}, {multi: false}];
					return Promise.resolve(update);
				})
				.then(function(update){
					//console.log('[4]:', update);
					return db.findOneAndUpdate.apply(db, update)
						.catch(function(error){
				...
				.then(function(raw){
					return out_send(record)
						.catch(function(error){
							//console.log('[5] send-er:', error);
							let update = [{_id: record._id}, {'$set': {error: {outResponse: error}, modified: Date.now()}}, {multi: false}];
							return Promise.reject(update);
						})
						.then(function(result){
							//console.log('[5] send-ok:', result);
							record.outResponse = result;
							let update = [{_id: record._id}, {'$set': {outResponse: result, modified: Date.now()}}, {multi: false}];
							return Promise.resolve(update);
						})
						.then(function(update){
							return db.findOneAndUpdate.apply(db, update)
		...
		.catch(function(update){
			...
			return update ? db.findOneAndUpdate.apply(db, update).then(rj) : rj();
		});
};
